...
/* V16 CLEAN DRIVE IMAGE OVERRIDES */
function v16Toast(msg){
  try { toast(msg); } catch(e) { console.log(msg); }
}

async function v16ImageFileToBase64Jpg(file, maxSize = 1000, quality = 0.84){
...
async function v16UploadImageToDrive(base64, filename){
...
async function loadPhoto(e){
  ...
  try{
    v16Toast("照片上傳中...");
    const base64 = await v16ImageFileToBase64Jpg(file, 900, 0.82);
    const url = await v16UploadImageToDrive(base64, "student_" + (s.name || "photo") + "_" + Date.now() + ".jpg");
    s.photo = url;
    persist();
    renderAll();
    v16Toast("照片已上傳到 Google Drive");
  }catch(err){
    console.error(err);
    alert("照片上傳失敗：\n" + (err.message || err));
    v16Toast("照片上傳失敗");
...
async function loadRewardPhoto(e){
  ...
  try{
    v16Toast("獎品照片上傳中...");
    const base64 = await v16ImageFileToBase64Jpg(file, 1000, 0.84);
    pendingRewardPhoto = await v16UploadImageToDrive(base64, "reward_" + Date.now() + ".jpg");
    renderRewardPhotoPreview(pendingRewardPhoto);
    v16Toast("獎品照片已上傳到 Google Drive");
  }catch(err){
    console.error(err);
    alert("獎品照片上傳失敗：\n" + (err.message || err));
    v16Toast("獎品照片上傳失敗");
...
function stripOversizedImages(obj){
  ...
  (copy.students || []).forEach(s => {
    if(s.photo && String(s.photo).startsWith("data:") && s.photo.length > 12000){
      s.photo = "";
      s.photoWarning = "舊版內嵌照片過大已移除，請用 V16 重新上傳到 Google Drive";
    }
  });
  (copy.rewards || []).forEach(r => {
    if(r.photo && String(r.photo).startsWith("data:") && r.photo.length > 15000){
      r.photo = "";
      r.photoWarning = "舊版內嵌獎品照片過大已移除，請用 V16 重新上傳到 Google Drive";
    }
  });
  return copy;
}
...
async function safeSaveToCloud(){
  ...
  try{
    persist();
    const local = normalize(stripOversizedImages(data));
    let cloud = normalize({});
...
async function forceSaveToCloud(){
  const url = localStorage.getItem(API_KEY);
  if(!url){ toast("請先貼上 Apps Script 網址"); setView("sync"); return; }
  if(!confirm("危險操作：強制覆蓋會用本機資料取代雲端。確定要強制覆蓋嗎？")) return;
  try{
    persist();
    const payload = stripOversizedImages(normalize(data));
...
/* END V16 CLEAN DRIVE IMAGE OVERRIDES */
